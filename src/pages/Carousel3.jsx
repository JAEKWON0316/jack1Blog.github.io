import React, { useState } from 'react';
import '../style/carousel.css';
import mysecurity01 from '../assets/spsecurity01.png';
import mysecurity02 from '../assets/mysecurity02.png';
import mysecurity03 from '../assets/mysecurity03.png';
import mysecurity04 from '../assets/mysecurity04.png';

const Carousel3 = ({ onClose }) => {
  // 이미지 배열과 상태 추가
  const images = [
    mysecurity01,
    mysecurity02,
    mysecurity03,
    mysecurity04 
  ];

  const [currentIndex, setCurrentIndex] = useState(0); // 현재 이미지 인덱스 상태

  const handleNext = () => {
    // 다음 이미지로 이동
    setCurrentIndex((prevIndex) => (prevIndex + 1) % images.length);
  };

  const handlePrev = () => {
    // 이전 이미지로 이동
    setCurrentIndex((prevIndex) => (prevIndex - 1 + images.length) % images.length);
  };

  return (
    <section id="Carousel3" className="carousel-container" onClick={onClose}>
      <div className='carousel_box' onClick={(e) => e.stopPropagation()}>
        <div className="carousel_img_box">
          <img
            src={images[currentIndex]} // 현재 인덱스의 이미지를 보여줌
            style={{ width: '100%', height: '100%', borderRadius: '1rem' }}
            alt="Project"
          />
        </div>
        <div className='carousel_img_move'>
          <button onClick={handlePrev}><i className="ri-arrow-drop-left-line"></i></button>
          <div className='carousel_move'>{currentIndex + 1} / {images.length}</div> {/* 현재 이미지 인덱스 표시 */}
          <button onClick={handleNext}><i className="ri-arrow-drop-right-line"></i></button>
        </div>
      </div>
    </section>
  );
};

export default Carousel3;